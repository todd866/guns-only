...
public readonly record struct BanditSkillProfile(
    ...
    /// Front-door rung. Competent is the floor: it can turn and it still fires on the body
    /// gate. Novice (2.40 G, no lookahead) is not on this ladder. Ace is rung 2 and above.
    public static PilotSkill ForRung(int rung) => rung switch {
        <= 0 => PilotSkill.Competent,
        1 => PilotSkill.Veteran,
        _ => PilotSkill.Ace,
    };

    /// Callers without a director decision get the cold floor, not Ace. The earned ladder
    /// lives on FightDirector.NextSpawn.
