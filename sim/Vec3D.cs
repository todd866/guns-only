...
public readonly record struct Vec3D(double X, double Y, double Z) {
    ...
    /// Standard right-handed determinant formula. NOTE: the sim world basis
    /// (X=east, Y=up, Z=north) is LEFT-handed, so physical direction products
    /// take reversed operand order: physical north x up = east == up.Cross(north).
    /// Frame-aware consumers (Doctrine.Geometry) must order operands accordingly.
