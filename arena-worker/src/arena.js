...
export const SCAFFOLDED_K = 8;
export const ELO_PER_SKILL_STEP = 200;
export const FUN_THRESHOLD = 0.45;
export const EXPLORATION_HUMAN_MATCHES = 12;
export const REMATCH_WINDOW_MS = 15 * 60 * 1000;
export const MINIMUM_RATED_DURATION_S = 20;
export const MINIMUM_ROUNDS_FOR_RATED = 1;
...
export const SEED_BOTS = Object.freeze([
  Object.freeze({
    botId: "bot:competent",
...
export function kFactor(gamesPlayed, { scaffolded = false, forceFullK = false } = {}) {
 ...
 */
export function applyElo(ratingA, ratingB, scoreA, kA, kB) {
...
export function handicappedSkillBlend(nativeSkill, botElo, humanElo) {
  const native = nativeSkillBlend(nativeSkill);
  const steps = (botElo - humanElo) / ELO_PER_SKILL_STEP;
  // Move only halfway toward Elo parity and cap a single match at two skill tiers. The same
  // bounded adjustment is authoritative in the Vercel and ASP.NET implementations; applying the
  // raw gap here made a seeded Machine collapse all the way to Novice against a new pilot.
...
export function profileAtBlend(blend) {
...
export function normalisePilotKey(pilotKey) {
...
export async function principalIdForPilotKey(pilotKey) {
...
export function newHumanPrincipal(principalId, nowMs) {
  return {
    kind: "human",
    principalId,
    elo: STARTING_ELO,
...
export function cloneSeedBot(seed, nowMs) {
  return {
    kind: "bot",
...
export function pickBot(bots, humanElo, random = Math.random) {
...
export function sanityAllowsRated(sanity, { scaffolded = false } = {}) {
 ...
 */
export function applyFunSignals(bot, {
  completed = false,
  earlyAbandon = false,
  rematch = false,
  againVote = 0,
...
export class ArenaStore {
  ...
  async ensureHuman(pilotKey) {
    ...
    const principalId = await principalIdForPilotKey(key);
