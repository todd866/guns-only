...
| Lift, drag, stability derivatives and control response | AT-802F parameter set in the shared coefficient model | provisional | The published wing anchors a cambered-wing surrogate with an explicit `CL0 = 0.92`. That same total coefficient drives lift, induced drag, stall incidence and protected controls; polar, inertia and control derivatives remain acceptance-tested gameplay values, not OEM tables or a certified training model. |
