...
Next candidate: Build 249 (local work); do not deploy until the complete gate is
...
| Cobra Canyon (`cobra-lab`, `/cobra-lab/`) | **production** | Standalone route | Authoritative AH-1G flight foundation, terrain/collision route authority, sim-owned ground war (contested sites, mutual faction fight, control balance, finite M134, Camp Ember rearm), copilot-gunner consent against ground units, compact flight diagnostics, and telemetry path | Fresh representative human desktop/touch ground-war sortie and complete green release gate |
