...
Next candidate: Build 301 (branch `fix/cobra-fob-battle`) — land Camp Ember FOB, airborne soft gates, denser Iron Bell fight.
...
| Cobra Canyon (`cobra-lab`, `/cobra-lab/`) | **production** | Standalone route | Hold the Bridge / Ember Run: AH-1G, sim-owned ground war, tip/hold win/lose, M134 + Camp Ember rearm, Tab/F gunner. Build 300: skids-on-pad Depart. Build 301 candidate: land spur FOB (not riverboat), airborne soft gates, denser Iron Bell destroyables | Owner verify land FOB open + airborne path + bridge fight. Longer arc: DCS-BS1-grade flight dynamics |
