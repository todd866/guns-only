...
**Cue rider.** `WeekendRideCueRider` reads the snapshot 30 ticks late, slews throttle 2/s, brake 4/s, steer 2/s, then `StepFixed` assisted. It does not read the centreline. Straight target 32 m/s so the opening straight still beats the hairpin's steady speed. Inside 150 m of an apex that is not the exit, it brakes toward the displayed `next_apex_mps`, not a private fraction of it and not from 280 m. Steer gain rises inside 40 m of an apex so the 12 m look, which is still almost straight on a 39 m hairpin, actually turns the bike. Exit speed stays at the displayed corner speed while the look is still off the bow. Pit approach is 12 m/s. Steer snaps to zero on the live in-pit cue so the 250 ms lag does not keep leaning the bike on the grass while it brakes through the 6 m/s tip. Tests: `CueRiderCompletesTheArcFromAColdStart`, `CueRiderBeatsASlowSeedAndStillStops`. Both finish with a legal stop, two laps, a cool-down, and a session under 8 minutes. The cold start stays on the paint for both flying laps, records zero tips and zero off-track time, two clean flying laps, and passes each apex within 4 m/s of the displayed board. The seeded case beats a 600 s lap.

**Apex board.** The slide limit is `sqrt(μ g r)` with surrogate μ 1.20 (`YzfR1Definition.TirePeakFrictionCoefficient`, `docs/vehicles/yamaha-yzf-r1/00-sources.md`) and g 9.80665. The published board uses `ApexFrictionUse` 0.36 of that friction, a speed factor of 0.60. Epistemic: provisional margin on a surrogate μ, not a measured lap. Braking to the raw slide limit inside 150 m ran the cue rider off the paint (0 laps, ~30 s off track). 0.64 of peak (speed factor 0.80) still did. 0.36 completes the arc and the rider is at the board through the apex. The page driver in `ride_cue_driver.js` uses the same 150 m policy.
...
### P1 cool-down, apex board, paint record

**Cool-down.** `JudgePitStop` finishes a post-checker stop only when `PitOpen` is armed. The box sits beside the stripe, so a legal stop in the first ticks after lap 2 was a normal completion with `CameInEarly` still false. An early stop during `flying` is unchanged. Test: `ImmediatePostCheckerPitEntryDoesNotFinish` (failed while the session finished from the box; passes with the session still `Active`, then finishes once the window is armed). `ATipRestoresOnTrackButTheLapCountersKeepIt` locks the reset: `IsOnTrack` is true again and `TipCount` is 1.

**Apex cue.** The cue rider and `ride_cue_driver.js` brake toward the displayed speed inside 150 m. The board is the margined steady speed above, not 55% of the slide limit and not a 22 m/s brake from 280 m. Tests: `brakes toward the displayed apex speed inside 150 m and not before` (node; failed while a 200 m cue still braked), `CueRiderCompletesTheArcFromAColdStart` (failed by 10.8 m/s at the apex before the rider followed the board; failed to finish on the raw slide limit). `FirstRealHeadingChangeIsAFewHundredMetresFromTheGrid` pins `sqrt(ApexFrictionUse)` times the slide limit.

**Paint.** The cold cue ride asserts `TipCount == 0`, `OffTrackSeconds == 0`, and `CleanFlyingLaps >= 2`. Sampling `IsOnTrack` before the step misses a tip, because `ResetToGrid` sets it true. Those counters were already zero on the previous rider; the new asserts are what make a later tip fail the build.

**Not done.** No new radio lines. No chase cam, no canopy, no change to the wordless front door or the first-person view. μ, the hairpin radius, and the reflex rates were not retuned. The 60 km/h pit limit was not promoted to measured. No release stamp, push, or deploy. Headless frames were not recaptured for this slice: the procedure change is the stop predicate and the board speed, and the kernel tests are the proof.
