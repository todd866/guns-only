...
function updateNavConsole(state) {
  ...
  const publishedSkinC = num("rapier_skin_temp_c");
  const skinC = finite(publishedSkinC) ? publishedSkinC : num("rapier_stagnation_temp_c");
  const wallC = num("rapier_adiabatic_wall_temp_c");
  const limitC = num("rapier_thermal_limit_c");
  const marginC = num("rapier_thermal_margin_c");
  set(navUi.skin, finite(skinC)
    ? `${Math.round(skinC)}°C SKIN${finite(limitC) ? ` / ${Math.round(limitC)}°C LIMIT` : ""}${finite(wallC) ? ` · WALL ${Math.round(wallC)}°C` : ""}`
    : "--",
    !finite(marginC) ? "unknown" : marginC < 0 ? "fault" : marginC < 100 ? "caution" : "normal");
