...
internal static class SnapshotProjection {
    ...
    static string BuildStateCore(SimulationSession session,
        ...
        return "{"
            ...
            + $"\"mission_feature_pack_id\":{JsonString(Environment.MissionFeaturePackId ?? "")},"
            + $"\"mission_feature_pack_sha256\":{JsonString(Environment.MissionFeaturePackSha256 ?? "")},"
            + $"\"mission_feature_pack_required\":{(Environment.MissionFeaturePackRequired ? "true" : "false")},"
            + $"\"lz_assessment_status\":\"{(Environment.MissionFeaturePackId is null ? "not_applicable" : "unassessed")}\","
            ...
            + $"\"rapier_design_dash_mach\":{RapierMissionDirector.MeasuredDashMach:F2},"
            ...
            + $"\"rapier_balloon_reaction_active\":{(Session.RapierBalloonReactionActive ? "true" : "false")},"
            + $"\"rapier_balloon_reaction_seconds\":{Session.RapierBalloonReactionSecondsRemaining:F2},"
            + $"\"rapier_balloon_payload_deployed\":{(Session.RapierBalloonPayloadDeployed ? "true" : "false")},"
            + $"\"rapier_balloon_carriers_remaining\":{(Session.RapierMissionAvailable && Session.Beat.ScriptedIntercept?.Job == RapierJobKind.Balloon ? Session.LiveOpponentCount : 0)},"
            ...
            + $"\"rapier_gate_face_x\":{Session.RapierGateFace.X:F4},"
            + $"\"rapier_gate_face_y\":{Session.RapierGateFace.Y:F4},"
            + $"\"rapier_gate_face_z\":{Session.RapierGateFace.Z:F4},"
            ...
            + $"\"rapier_inlet_unstart\":{(_player.InletUnstarted ? "true" : "false")},"
            + $"\"rapier_over_q\":{(_player.OverDynamicPressure ? "true" : "false")},"
            ...
            + $"\"gunnery_assist_status\":{SnapshotJson.JsonString(gunneryPitchAssist.Status)},"
            + $"\"gunnery_assist_status_code\":{gunneryPitchAssist.StatusCode},"
            + $"\"gunnery_pitch_error_deg\":{gunneryPitchAssist.PitchLeadErrorRad * 57.29577951308232:F3},"
            + $"\"gunnery_lateral_error_deg\":{gunneryPitchAssist.LateralLeadErrorRad * 57.29577951308232:F3},"
            ...
            + $"\"gunnery_assist_authority_01\":{gunneryPitchAssist.Authority01:F3},"
            + $"\"gunnery_roll_assist\":{gunneryPitchAssist.RollCorrection:F4},"
            + $"\"gunnery_yaw_assist\":{gunneryPitchAssist.YawCorrection:F4},"
            + $"\"gunnery_time_to_pass_s\":{FiniteNumberJson(gunneryPitchAssist.TimeToPassSeconds)},"
            ...
            + $"\"assisted_flight\":{(Session.AssistedFlight ? "true" : "false")},"
            + $"\"assisted_speed_bias_kts\":{Session.AssistedSpeedBiasKts},"
