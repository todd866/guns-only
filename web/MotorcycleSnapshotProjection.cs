...
internal static class MotorcycleSnapshotProjection
{
    ...
    static object BuildState(WeekendRideMissionRuntime runtime, MotorcycleControlMode controlMode)
    {
        ...
        return new {
            ...
            progress_m = snap.ProgressM,
            session_s = snap.SessionSeconds,
            next_apex_m = snap.NextApexDistanceM,
            next_apex_mps = snap.NextApexSteadySpeedMps,
            next_apex_exit = snap.NextApexIsExit,
            session_leg = snap.SessionLeg == WeekendRideSessionLeg.Cooldown ? "cooldown" : "flying",
            pit_open = snap.PitOpen,
            look_ahead_lateral_m = snap.LookAheadLateralM,
            in_pit = snap.InPit,
            pit_entry_legal = snap.PitEntryLegal,
            legal_stop = snap.LegalStop,
            came_in_early = snap.CameInEarly,
            hot_pit_entry = snap.HadHotPitEntry,
            tip_count = snap.TipCount,
            show_apex_speed = snap.ShowApexSpeed,
            reflex_gain = snap.ReflexGain,
            clean_flying_laps = snap.CleanFlyingLaps,
            clean_lap_s = snap.LastCleanFlyingLapSeconds,
